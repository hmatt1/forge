...
public class ProtoMapper {
    ...
    public static GameState mapGameState(Game game, Player perspective, String prompt, List<Action> availableActions) {
        GameState.Builder builder = GameState.newBuilder()
                ...
                .setCurrentStep(game.getPhaseHandler().getPhase().toString())
    ...
    private static CardState mapCard(Card card) {
        ...
        // Map colors
        for (forge.card.MagicColor.Color c : card.getColor()) {
            builder.addColors(c.toString());
        }
        
        // Map types
        for (forge.card.CardType.Supertype st : card.getType().getSupertypes()) {
            builder.addTypes(st.toString());
        }
        for (forge.card.CardType.CoreType ct : card.getType().getCoreTypes()) {
            builder.addTypes(ct.toString());
        }
        for (String sub : card.getType().getSubtypes()) {
            builder.addTypes(sub);
        }
